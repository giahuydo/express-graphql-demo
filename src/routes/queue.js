 ...
 *     responses:
 *       200:
 *         description: Test notification job added successfully
 *         content:
 *           application/json:
 *             schema:
 *               type: object
...
router.post('/test-notification', requireAdmin, async (req, res) => {
  try {
    const { email, name, subject, message, type = 'info' } = req.body;
    
    if (!email || !name || !subject || !message) {
      return res.status(400).json({
        success: false,
        message: 'Email, name, subject, and message are required'
      });
    }

    const job = await queueService.addNotificationEmailJob({
      email,
      name,
      subject,
      message,
    ...
    res.json({
      success: true,
      message: 'Test notification job added successfully',
      jobId: job.id,
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    console.error('❌ Failed to add test notification job:', error);
    res.status(500).json({
      success: false,
      message: 'Failed to add test notification job',
      error: error.message
    });
  }
});
